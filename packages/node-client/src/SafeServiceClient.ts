...
class SafeServiceClient implements SafeTransactionService {
   ...
   * @returns The information and configuration of the service
   */
  async getServiceInfo(): Promise<SmartAccountInfoResponse> {
    return sendRequest({
  ...
  async addSafeDelegate({
    ...
    label,
    signer
  }: SmartAccountDelegateConfig): Promise<SmartAccountDelegate> {
    if (safe === '') {
      throw new Error('Invalid Safe address')
    }
    if (delegate === '') {
    ...
    const body: SmartAccountDelegate = {
      safe: safeAddress,
   ...
   */
  async getSafeCreationInfo(safeAddress: string): Promise<SmartAccountCreationInfoResponse> {
    if (safeAddress === '') {
      throw new Error('Invalid Safe address')
    }
    const { address } = await this.#ethAdapter.getEip3770Address(safeAddress)
