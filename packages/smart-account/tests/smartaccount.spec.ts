...
describe('Wallet integration', function () {
  ...
  describe('Smart account usage and basic actions', () => {
    ...
    beforeEach(async () => {
      ...
      const [smartWallet, walletFactory, multiSend, multiSendCallOnly] = await deployWalletContracts(ethnode.signer);
    ...
    it('Should be able to sign transaction', async () => {
      ...
      const smartAccountTransaction:WalletTransaction = await smartAccount.createSmartAccountTransaction(tx);
    ...
    it('Should be able to send transaction', async () => {
      ...
      const response:TransactionResponse = await smartAccount.sendTransaction(smartAccountTransaction);

      const receipt: TransactionReceipt = await response.wait(1);
      expect(receipt.status).to.be.equal(1); 
      console.log('balance after ', await ethnode.provider?.getBalance(smartAccountAddress))
      expect((await ethnode.provider?.getBalance(smartAccountAddress))?.toString()).to.be.equal(ethers.utils.parseEther("0.5").toString())
      }
    })
  })
})
