...
const matchError = (message: string): null | KnownError =>
  knownErrors.find((knownError: KnownError) => message.toLowerCase().indexOf(knownError.regex) > -1) ?? null;
...
export const getAAError = async (message: string, service?: Service) => {
  ...
  const metaMessages = matchedError ? buildErrorStrings(matchedError, service) : [];
