import { getAddress, isHex, type ByteArray, type Hex, type WalletClient } from "viem";
...
export class WalletClientSigner implements SmartAccountSigner<WalletClient> {
  ...
  readonly signMessage: (message: string | Hex | ByteArray) => Promise<`0x${string}`> = async (message) => {
  ...
  signTypedData: (params: SignTypedDataParams) => Promise<`0x${string}`> = async (params) => {
    const account = this.inner.account ?? (await this.getAddress());
