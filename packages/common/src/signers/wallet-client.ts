import { getAddress, type Hex, type SignableMessage, type TypedData, type TypedDataDefinition, type WalletClient } from "viem";
...
export class WalletClientSigner implements SmartAccountSigner<WalletClient> {
  ...
  readonly signMessage: (message: SignableMessage) => Promise<`0x${string}`> = async (message) => {
    const account = this.inner.account ?? (await this.getAddress());

    return this.inner.signMessage({ message, account });
  };

  signTypedData = async <const TTypedData extends TypedData | { [key: string]: unknown }, TPrimaryType extends string = string>(
