...
describe("Account Tests", () => {
  ...
  describe("6492 signature verification", () => {
    it("should verifySignature of deployed", async () => {
      const {
        whale: { viemWallet: signer },
        bundlerUrl,
        publicClient,
    ...
    it("should verifySignature of not deployed", async () => {
      const {
        whale: { viemWallet: signer },
        bundlerUrl,
        publicClient,
      ...
      const signature = await smartAccount.signMessage(message);
      console.log(await smartAccount.getAddress(), "address");
      const isVerified = await publicClient.verifyMessage({
        address: await smartAccount.getAddress(),
        message,
        signature,
