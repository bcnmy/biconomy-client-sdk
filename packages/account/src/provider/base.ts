...
import type { ISmartContractAccount, SignTypedDataParams } from "../account/types";
...
export class SmartAccountProvider<TTransport extends SupportedTransports = Transport>
  ...
  implements ISmartAccountProvider<TTransport>
{
  ...
  constructor(config_: SmartAccountProviderConfig<TTransport>) {
    const config = createSmartAccountProviderConfigSchema<TTransport>().parse(config_);
  ...
  sendTransaction = async (request: RpcTransactionRequest, overrides?: UserOperationOverrides): Promise<Hash> => {
    const uoStruct = await this.buildUserOperationFromTx(request, overrides);

    const { hash } = await this._sendUserOperation(uoStruct);

    return this.waitForUserOperationTransaction(hash as Hash);
  };

  buildUserOperationFromTx = async (request: RpcTransactionRequest, overrides?: UserOperationOverrides): Promise<UserOperationStruct> => {
    if (!request.to) {
  ...
  buildUserOperationFromTxs = (requests: RpcTransactionRequest[], overrides?: UserOperationOverrides) => {
    ...
    const maxFeePerGas =
      overrides?.maxFeePerGas != null
        ...
        : bigIntMax(...requests.filter((x) => x.maxFeePerGas != null).map((x) => fromHex(x.maxFeePerGas!, "bigint")));
    const maxPriorityFeePerGas =
      overrides?.maxPriorityFeePerGas != null
        ? overrides?.maxPriorityFeePerGas
        : bigIntMax(...requests.filter((x) => x.maxPriorityFeePerGas != null).map((x) => fromHex(x.maxPriorityFeePerGas!, "bigint")));
  ...
  sendTransactions = async (requests: RpcTransactionRequest[], overrides?: UserOperationOverrides) => {
  ...
  waitForUserOperationTransaction = async (hash: Hash): Promise<Hash> => {
    for (let i = 0; i < this.txMaxRetries; i++) {
      const txRetryIntervalWithJitterMs = this.txRetryIntervalMs * Math.pow(this.txRetryMulitplier, i) + Math.random() * 100;

      await new Promise((resolve) => setTimeout(resolve, txRetryIntervalWithJitterMs));
      const receipt = await this.getUserOperationReceipt(hash as `0x${string}`).catch((e) => {
  ...
  dropAndReplaceUserOperation = async (uoToDrop: UserOperationRequest, overrides?: UserOperationOverrides): Promise<SendUserOperationResult> => {
    const uoToSubmit = {
  ...
  checkGasSponsorshipEligibility = async (
    ...
    return this.buildUserOperation(data, overrides)
      .then(
        (userOperationStruct: UserOperationStruct) => userOperationStruct.paymasterAndData !== "0x" && userOperationStruct.paymasterAndData !== null,
      )
      .catch(() => false);
  };

  private _runMiddlewareStack = async (uo: Deferrable<UserOperationStruct>, overrides?: UserOperationOverrides) => {
    const result = await asyncPipe(
      this.dummyPaymasterDataMiddleware,
      this.feeDataGetter,
      this.gasEstimator,
      this.customMiddleware ?? noOpMiddleware,
      overrides?.paymasterAndData != null ? this.overridePaymasterDataMiddleware : this.paymasterDataMiddleware,
  ...
  private _sendUserOperation = async (uoStruct: UserOperationStruct) => {
    ...
    request.signature = (await this.account.signUserOperationHash(
      getUserOperationHash(request, this.getEntryPointAddress(), BigInt(this.chain.id)),
    )) as `0x${string}`;
  ...
  readonly dummyPaymasterDataMiddleware: AccountMiddlewareFn = async (struct, _overrides, _feeOptions) => {
    struct.paymasterAndData = "0x";
    return struct;
  };

  readonly overridePaymasterDataMiddleware: AccountMiddlewareFn = async (struct, overrides, _feeOptions) => {
    struct.paymasterAndData = overrides?.paymasterAndData != null ? overrides?.paymasterAndData : "0x";
    return struct;
  };

  readonly paymasterDataMiddleware: AccountMiddlewareFn = async (struct, _overrides, _feeOptions) => {
    struct.paymasterAndData = "0x";
    return struct;
  };

  readonly gasEstimator: AccountMiddlewareFn = async (struct, overrides, feeOptions) => {
  ...
  readonly feeDataGetter: AccountMiddlewareFn = async (struct, overrides, feeOptions) => {
    ...
    let maxPriorityFeePerGas: BigNumberish = await this.rpcClient.estimateMaxPriorityFeePerGas();
    ...
    let maxFeePerGas: BigNumberish = feeData.maxFeePerGas - feeData.maxPriorityFeePerGas + BigInt(maxPriorityFeePerGas);
  ...
  connect = <TAccount extends ISmartContractAccount>(
    fn: (provider: PublicErc4337Client<TTransport> | PublicErc4337Client<HttpTransport>) => TAccount,
  ): IsUndefined<TAccount["providerDecorators"]> extends true
    ? this & { account: TAccount }
    : this & { account: TAccount } & ReturnType<NoUndefined<TAccount["providerDecorators"]>> => {
    ...
    if (this.entryPointAddress && account.getEntryPointAddress() !== this.entryPointAddress) {
      throw new Error(
    ...
    if (this.rpcClient.transport.type === "http") {
      const { url = this.chain.rpcUrls.default.http[0], fetchOptions } = this.rpcClient.transport as ReturnType<HttpTransport>["config"] &
    ...
    account.getAddress().then((address) => this.emit("accountsChanged", [address]));

    if (account.providerDecorators) {
      this.extend(account.providerDecorators);
    }

    return this as unknown as IsUndefined<TAccount["providerDecorators"]> extends true
      ? this & { account: TAccount }
      : this & { account: TAccount } & ReturnType<NoUndefined<TAccount["providerDecorators"]>>;
  };
  ...
  getEntryPointAddress = (): Address => {
    return this.entryPointAddress ?? this.account?.getEntryPointAddress() ?? getDefaultEntryPointAddress(this.chain);
  };
   ...
   */
  upgradeAccount = async (upgradeTo: UpgradeToData, waitForTxn = false): Promise<Hash> => {
    if (!this.isConnected()) {
    ...
    const encodeUpgradeData = await this.account.encodeUpgradeToAndCall(accountImplAddress, initializationData);
  ...
  private overrideMiddlewareFunction = (override: AccountMiddlewareOverrideFn): AccountMiddlewareFn => {
