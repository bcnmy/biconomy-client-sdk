import { BatchUserOperationCallData, SendUserOperationResult, SmartAccountProvider, UserOperationCallData } from "@alchemy/aa-core";
...
export class BiconomyAccountProvider extends SmartAccountProvider<HttpTransport> {
  sendUserOperation = async (data: UserOperationCallData | BatchUserOperationCallData): Promise<SendUserOperationResult> => {
    ...
    const userOp = await (this.account as BiconomySmartAccountV2).buildUserOp(tx);
