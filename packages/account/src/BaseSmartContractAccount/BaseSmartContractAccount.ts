...
import { getContract, http, trim, type GetContractReturnType, type Hash, type Hex, type PublicClient, createPublicClient } from "viem";
import { EntryPointAbi } from "./abis/EntryPointAbi.js";
import type { ISmartContractAccount, SignTypedDataParams, BatchUserOperationCallData, BasSmartContractAccountProps } from "./types.js";
import { DEFAULT_ENTRYPOINT_ADDRESS } from "@biconomy/modules";
import { Logger, SmartAccountSigner } from "@biconomy/common";
import { getChain, wrapSignatureWith6492 } from "../utils/Utils.js";
...
export abstract class BaseSmartContractAccount<TSigner extends SmartAccountSigner = SmartAccountSigner> implements ISmartContractAccount<TSigner> {
  ...
  constructor(params: BasSmartContractAccountProps) {
    this.entryPointAddress = params.entryPointAddress ?? DEFAULT_ENTRYPOINT_ADDRESS;
  ...
  async signMessageWith6492(msg: string | Uint8Array): Promise<`0x${string}`> {
    const [isDeployed, signature] = await Promise.all([this.isAccountDeployed(), this.signMessage(msg)]);
   ...
   */
  async signTypedDataWith6492(params: SignTypedDataParams): Promise<`0x${string}`> {
    const [isDeployed, signature] = await Promise.all([this.isAccountDeployed(), this.signTypedData(params)]);
   ...
   */
  async encodeBatchExecute(_txs: BatchUserOperationCallData): Promise<`0x${string}`> {
   ...
   */
  protected async parseFactoryAddressFromAccountInitCode(): Promise<[Address, Hex]> {
    const initCode = await this._getAccountInitCode();
  ...
  private async create6492Signature(isDeployed: boolean, signature: Hash): Promise<Hash> {
    if (isDeployed) {
      return signature;
    }
