...
import type { UserOperationEstimateGasResponse, UserOperationReceipt, UserOperationRequest, UserOperationResponse } from "../types";
import { VERSION } from "../version";
import type { Erc4337RpcSchema, PublicErc4337Client } from "./types.js";

export const erc4337ClientActions = (client: Client) => {
  const clientAdapter = client as Client<Transport, Chain, undefined, [...PublicRpcSchema, ...Erc4337RpcSchema], PublicActions>;
...
export const createPublicErc4337FromClient: <T extends Transport | FallbackTransport = Transport>(
  client: PublicClient<T, Chain>,
) => PublicErc4337Client<T> = <T extends Transport | FallbackTransport = Transport>(client: PublicClient<T, Chain>): PublicErc4337Client<T> => {
