...
import { concatHex, encodeFunctionData, hexToBytes, type FallbackTransport, type Hex, type Transport } from "viem";
import { SimpleAccountAbi } from "../abi/SimpleAccountAbi";
import { SimpleAccountFactoryAbi } from "../abi/SimpleAccountFactoryAbi";
...
export class SimpleSmartContractAccount<TTransport extends Transport | FallbackTransport = Transport> extends BaseSmartContractAccount<
  TTransport,
  SmartAccountSigner
> {
  ...
  override async encodeBatchExecute(txs: BatchUserOperationCallData): Promise<`0x${string}`> {
