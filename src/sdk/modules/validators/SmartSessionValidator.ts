...
import { type ActionData, ChainDigest, ChainSession, encodeSmartSessionSignature, hashChainSessions, type PolicyData, type Session, SmartSessionMode } from "@rhinestone/module-sdk"
import { CreateSessionWithEnableModeResponse, type ActionConfig, type CreateSessionDataParams, type CreateSessionDataResponse, type ModuleInfo, type ParamRule } from "../utils/Types.js"
import { smartSessionAbi, universalActionPolicyAbi } from "../utils/abi.js"
import { getPermissionId, getSessionDigest, getSessionNonce, toActionConfig } from "../index.js"
...
export class SmartSessionValidator extends BaseValidationModule {
    ...
    // Note:
    // for enable mode we will also need enableSessionData
    // that looks like
    /*
    enableSessionData: {
        enableSession: {
    ...
    */
    ...
    createSessionWithEnableMode = async (
        sessionRequestedInfo: CreateSessionDataParams[],
    ): Promise<CreateSessionWithEnableModeResponse | void> => {
        const sessions: Session[] = [];
        const permissionIds: Hex[] = [];
        ...
        for (const sessionInfo of sessionRequestedInfo) {
            ...
            const session: Session = {
              ...
              const sessionNonce = await getSessionNonce({
                client: this.client,
                accountAddress: this.currentSA.address,
                permissionId,
              })
              ...
              const sessionDigest = await getSessionDigest({ client: this.client, accountAddress: this.currentSA.address, session: session, permissionId: permissionId, mode: SmartSessionMode.UNSAFE_ENABLE }) as Hex

              // Iterate over chainIds and create chainDigest objects
              for (const chainId of sessionInfo.chainIds || []) {
                chainDigests.push({
                ...
                chainSessions.push({
                  ...
                  session: {
                    ...session,
                    account: this.currentSA.address,
                    smartSession: addresses.SmartSession,
                    mode: 1,
                    nonce: sessionNonce, // above session nonce is used here
                  },
                });
              }
        }

        const permissionEnableHash = hashChainSessions(chainSessions) as Hex

        return { permissionIds: permissionIds, sessions: sessions, chainDigests: chainDigests, permissionEnableHash: permissionEnableHash }
    }
}
